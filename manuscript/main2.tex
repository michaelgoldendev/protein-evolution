...
Additionally note, although the summation in Equation~\ref{eq:ratematrix} appears to involve an exponential number of terms, most terms are equal to zero, except those that differ from $P_a$ at one position. Furthermore, an amino acid transition and a hidden state transition are not permitted to occur simulatenously, further reducing the number of terms that are summed ($19n$ amino acid terms, and $(h-1)n$ hidden state terms).
...
\begin{align}
p(\mathcal{D}_d|\mathcal{B}_d,\Upsilon_{d},\theta)=\prod_{b \in \mathcal{B}_d} p\big(X_{b}^{i}\,|\,H_{b}^{i},S_{b}^{i},\theta\big) \times \notag\\ 
\big[ e^{q_{last}t_{b}}\prod_{i\rightarrow j}q_{ii}e^{-q_{ii}(t_{b}-t_{b})}q_{ij}/q_{ii} \big]
\end{align}
...
\begin{align*}
Z_{d}^{(k)}\sim p(\mathcal{B}_d,\Upsilon_{d}|\mathcal{D}_d,\mathcal{T}_d,\Psi^{(k)}).
\end{align*}
