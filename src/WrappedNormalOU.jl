module WrappedNormalOU
  ...
  mutable struct WrappedNormalOUNode
    x::Array{Float64,1}
    t::Float64
    mu::Array{Float64,1}
  ...
  function get_parameters(node::WrappedNormalOUNode)
    return Float64[node.mu[1], node.mu[2], node.alpha[1], node.alpha[2], node.alpha[3], node.sigma[1], node.sigma[2]]
  end

  function set_parameters(node::WrappedNormalOUNode, params::Array{Float64,1}, t::Float64=node.t)
    set_parameters(node, t, params[1], params[2], params[3], params[4], params[5], params[6], params[7])
  end
  ...
  function set_parameters(node::WrappedNormalOUNode, mu::Array{Float64,1}, alpha::Array{Float64,1}, sigma::Array{Float64,1})
      ...
      node.twokpi = range(-node.maxK*2.0*pi, stop=node.maxK*2.0*pi, length=node.lk)
      ...
      node.s = tr(node.A) / 2.0
      s = node.s
      node.q = sqrt(abs(det(node.A - s * Matrix{Float64}(I, 2, 2))))
  ...
  function set_parameters(node::WrappedNormalOUNode, t::Float64)
      if node.t != t
        ...
        cqt = (eqt + 1.0/eqt) / 2.0
        sqt = (eqt - 1.0/eqt) / 2.0
        node.ExptA = ((cqt + s * sqt / q) * Matrix{Float64}(I, 2, 2) - (sqt / q * node.A)) / est
  ...
  function sampstat(node::WrappedNormalOUNode, rng::AbstractRNG)
      ...
      x = transpose(x)*chol(Hermitian(invASigma))
      x += transpose(node.mu)
  ...
  function samptrans(node::WrappedNormalOUNode, rng::AbstractRNG, x0::Array{Float64,1}, t::Float64)
    ...
    try
      ...
      try
        mineig = minimum(eigvals(node.Gammat))
        M = node.Gammat - (mineig * Matrix{Float64}(I, node.Gammat, node.Gammat))
        x = randn(rng,2)
        x = transpose(x)*chol(M)
        x += mutx
        x -= floor((x + pi) / (2.0 * pi)) * (2.0 * pi)
        return x
      catch
...
println(loglikwndtpd(me, 0.4, -1.0,1.0, 0.5))
println(loglikwndstat(me, 0.4, -1.0))
println(loglikwndstat(me, 1.0, 0.5))
=#
#=
