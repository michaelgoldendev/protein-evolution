...
mutable struct ModelParams
    ...
    function ModelParams(aminoacidQ::Array{Float64,2}, numhiddenstates::Int,mu::Float64=1.0,hiddenmu::Float64=1.0)
		...
		matrixcache = Dict{Tuple{Int,Int,Int}, Tuple{Array{Float64,2},Array{Complex{Float64},2},Array{Complex{Float64},1},Array{Complex{Float64},2}}}()
        new(20,aminoacidQ,LGexchangeability,numhiddenstates,initialprobs,transitioncounts,transitionprobs,transitionrates,hiddennodes,mu,hiddenmu,matrixcache,1.0,1.0)
    end
end
...
function getaaentry(modelparams::ModelParams, prevh_hmm::Int, nexth_hmm::Int, h::Int, prevaa::Int, curraa::Int)
	if prevaa != curraa
		...
		=#
		return modelparams.scalingfactor*modelparams.mu*modelparams.aa_exchangeablities[prevaa,curraa]*modelparams.hiddennodes[h].aa_node.probs[curraa]
	else
		q = 0.0
...
function train(numhiddenstates::Int=10)
	...
	outputmodelname = string("_h.",modelparams.numhiddenstates,"_learnrates.",learnrates,".revert.optimize")
	...
	for iter=1:1000
		...
		updatehiddenrates = iter % 6 == 1
		updateaarates = iter % 6 == 4
		if learnrates && updatehiddenrates || updateaarates
			for (proteins,nodelist,json_family,sequences) in trainingexamples
				numcols = length(proteins[1])
		...
		#=
		if iter % 6 == 0
			temp = copy(modelparams.aa_exchangeablities)
			augmentedll_old,observationll_old = calculateloglikelihood(modelparams, trainingexamples)
			modelparams.aa_exchangeablities = aatransitionrates
			augmentedll_prop,observationll_prop = calculateloglikelihood(modelparams, trainingexamples)
			if augmentedll_prop > augmentedll_old
				
			else 
				modelparams.aa_exchangeablities = copy(temp)
			end

			optimizeaarates(trainingexamples, modelparams,0)
		end

		
		if iter % 6 == 1
			temp = copy(modelparams.transitionrates)
			augmentedll_old,observationll_old = calculateloglikelihood(modelparams, trainingexamples)
			for h=1:modelparams.numhiddenstates
				modelparams.transitionrates[h,h] = 0.0			
				for h2=1:modelparams.numhiddenstates
			...
			if augmentedll_prop > augmentedll_old
				...
				modelparams.transitionrates = copy(temp)
			end

			optimizehiddenrates(trainingexamples, modelparams,0)
		end=#
		#=
		if (iter) % 5 == 0
...
function getaaarray(modelparams::ModelParams)
	x = Float64[]
	for h=1:20
		for h2=h+1:20
			push!(x, modelparams.aa_exchangeablities[h,h2])
		end
	end
	return x
end

function setaaarray(modelparams::ModelParams, x::Array{Float64,1})
	index = 1
	for h=1:20
		for h2=h+1:20
			modelparams.aa_exchangeablities[h,h2] = x[index]
			modelparams.aa_exchangeablities[h2,h] = x[index]			
			index += 1
		end
	end
	for h=1:20
		modelparams.aa_exchangeablities[h,h] = 0.0
		for h2=1:20
			if h != h2
				modelparams.aa_exchangeablities[h,h] -= modelparams.aa_exchangeablities[h,h2]
			end
		end
	end
	return modelparams
...
function aaratehelper(x::Array{Float64,1}, trainingexamples::Array{Tuple,1}, modelparams::ModelParams,extra)
	setaaarray(modelparams,x)
	augmentedll = 0.0
	for (proteins,nodelist) in trainingexamples		
		numcols = length(proteins[1])
		augmentedll += augmentedloglikelihood(nodelist, Int[col for col=1:numcols], modelparams)
	end
	println("AA ",augmentedll,"\t",x)
	return augmentedll
end

function optimizeaarates(trainingexamples::Array{Tuple,1}, modelparams::ModelParams, extra)
	numparams = div(20*20-20,2)
	opt = Opt(:LN_COBYLA, numparams)
    localObjectiveFunction = ((param, grad) -> aaratehelper(param, trainingexamples, modelparams,extra))
    lower = ones(Float64, numparams)*1e-200
    upper = ones(Float64, numparams)*1e4
    lower_bounds!(opt, lower)
    upper_bounds!(opt, upper)
    xtol_rel!(opt,1e-5)
    maxeval!(opt, numparams*2)
    max_objective!(opt, localObjectiveFunction)
    (minf,minx,ret) = optimize(opt, getaaarray(modelparams))
    setaaarray(modelparams,minx)
	return minx
end
...
function gethiddenarray(modelparams::ModelParams)
	x = Float64[]
	for h=1:modelparams.numhiddenstates
		for h2=h+1:modelparams.numhiddenstates
...
function sethiddenarray(modelparams::ModelParams, x::Array{Float64,1})
	...
	for h=1:modelparams.numhiddenstates
		for h2=h+1:modelparams.numhiddenstates
			...
			modelparams.transitionrates[h2,h] = x[index]			
			index += 1
		end
	end
	for h=1:modelparams.numhiddenstates
...
function optimizehiddenrates(trainingexamples::Array{Tuple,1}, modelparams::ModelParams, extra)
	numparams = div(modelparams.numhiddenstates*modelparams.numhiddenstates-modelparams.numhiddenstates,2)
	opt = Opt(:LN_COBYLA, numparams)
...
function infer(;modelfile="model_h.10_learnrates.true.revert.optimize.model", samplebranchlengths::Bool=false)
