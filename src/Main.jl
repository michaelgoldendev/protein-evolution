...
mutable struct ModelParams
    ...
    function ModelParams(aminoacidQ::Array{Float64,2}, numhiddenstates::Int,mu::Float64=1.0,hiddenmu::Float64=1.0)
		...
		numanglecats = 200
        new(20,aminoacidQ,LGexchangeability, rate_alpha, numrates, rates,rate_freqs,numhiddenstates,initialprobs,transitioncounts,transitionprobs,transitionrates,hiddennodes,mu,hiddenmu,matrixcache,1.0,1.0,true,true,1, ones(Float64,numhiddenstates), true, true, numanglecats)
    end
end

mutable struct OUDiffusionCache
	phimatrixcache::Dict{Tuple{Int,Float64},Tuple{Array{Float64,2},Float64}}
	psimatrixcache::Dict{Tuple{Int,Float64},Tuple{Array{Float64,2},Float64}}

	phistatcache::Dict{Int,Array{Float64,1}}
	psistatcache::Dict{Int,Array{Float64,1}}

	function OUDiffusionCache()
		phimatrixcache = Dict{Tuple{Int,Float64},Tuple{Array{Float64,2},Float64}}()
		psimatrixcache = Dict{Tuple{Int,Float64},Tuple{Array{Float64,2},Float64}}()
		phistatcache = Dict{Int,Array{Float64,1}}()
		psistatcache = Dict{Int,Array{Float64,1}}()
		new(phimatrixcache, psimatrixcache,phistatcache,psistatcache)
	end
end

function getphilogstat(modelparams::ModelParams, oudiffusioncache::OUDiffusionCache, h::Int)
	if haskey(oudiffusioncache.phistatcache, h)
		return oudiffusioncache.phistatcache[h]
	else
		v = getlogstatrow(modelparams.hiddennodes[h].phi_diffusion_node, modelparams.numcats)
		oudiffusioncache.phistatcache[h] = v
		return v
	end
end

function getpsilogstat(modelparams::ModelParams, oudiffusioncache::OUDiffusionCache, h::Int)
	if haskey(oudiffusioncache.psistatcache, h)
		return oudiffusioncache.psistatcache[h]
	else
		v = getlogstatrow(modelparams.hiddennodes[h].psi_diffusion_node, modelparams.numcats)
		oudiffusioncache.psistatcache[h] = v
		return v
	end
end

function getphitransitionmatrix(modelparams::ModelParams, oudiffusioncache::OUDiffusionCache, h::Int, node::TreeNode, phiobserved::Bool)
	t = node.branchlength
	if phiobserved
		t += modelparams.hiddennodes[h].phi_diffusion_node.obserror
	end
	key = (h,t)
	if haskey(oudiffusioncache.phimatrixcache, key)
		return oudiffusioncache.phimatrixcache[key]
	else
		phimatrix,philogconst = gettransitionmatrix(modelparams.hiddennodes[h].phi_diffusion_node, modelparams.numcats, node.branchlength, phiobserved)		
		v = (phimatrix,philogconst)
		oudiffusioncache.phimatrixcache[key] = v
		return v
	end
end

function getpsitransitionmatrix(modelparams::ModelParams, oudiffusioncache::OUDiffusionCache, h::Int, node::TreeNode, psiobserved::Bool)
	t = node.branchlength
	if psiobserved
		t += modelparams.hiddennodes[h].psi_diffusion_node.obserror
	end
	key = (h,t)
	if haskey(oudiffusioncache.psimatrixcache, key)
		return oudiffusioncache.psimatrixcache[key]
	else
		psimatrix,psilogconst = gettransitionmatrix(modelparams.hiddennodes[h].psi_diffusion_node, modelparams.numcats, node.branchlength, psiobserved)		
		v = (psimatrix,psilogconst)
		oudiffusioncache.psimatrixcache[key] = v
		return v
	end
end
...
mutable struct Sample 
	name::String
	nodedata::Array{Array{Tuple,1},1}
	...
	function Sample(name::String, modelparams::ModelParams)
		new(name, Array{Tuple,1}[], Array{Tuple{Float64,Float64},1}[], Int[], Int[], Dict{String,Any}(), modelparams)
	end
end
...
mutable struct AugmentedNodeData <: NodeData
	...
	angleloglikelihood::Array{Float64,1}

	AugmentedNodeData(numcols::Int) = new(BranchPath(numcols),BranchPath(numcols),BranchPath(numcols),BranchPath(numcols),1, Protein(numcols),1.0, false, zeros(Float64,numcols), zeros(Float64,numcols)) 
	AugmentedNodeData(branchpath::BranchPath, aabranchpath::BranchPath, ratesbranchpath::BranchPath, jointbranchpath::BranchPath, dummy::Int) = new(branchpath, aabranchpath, ratesbranchpath, jointbranchpath, dummy, Protein(),1.0,false, zeros(Float64,1), zeros(Float64,length(aabranchpath.paths)))
end
...
function felsensteinresample_hidden(rng::AbstractRNG, nodelist::Array{TreeNode,1}, selcolin::Int, cols::Array{Int,1}, aacol::Int, modelparams::ModelParams, sample::Bool=true, useoldsampling::Bool=false)
	#selcol = findfirst(x -> x == selcolin, cols)
	selcol = selcolin
...
function felsensteinresample_aa(rng::AbstractRNG, nodelist::Array{TreeNode,1}, cols::Array{Int,1}, aacol::Int, modelparams::ModelParams, sample::Bool=true, useoldsampling::Bool=false)
	...
	while length(stack) > 0
		...
		if isleafnode(node)
			...
			if cont
        		...
        		likelihoods[nodeindex, :] = (lefttransprobs*likelihoods[leftchildindex,:]).*(righttransprobs*likelihoods[rightchildindex,:])
        		if modelparams.hidden_conditional_on_aa
        			likelihoods[nodeindex, :] = likelihoods[nodeindex, :].*observationlikelihood_aa(node.data.protein, aacol, node.data.branchpath.paths[aacol][end], modelparams)
        		end
...
function observationlikelihood_aa(protein::Protein, col::Int, h::Int, modelparams::ModelParams)
	if 1 <= col <= length(protein.sites)
		...
		return ones(Float64, modelparams.alphabet)
	end
end
	...
	for col=1:numcols
		...
		if aa != '-'
			...
			if modelparams.use_diffusion
				site.phi = node.data.protein.sites[col].phi
				site.psi = node.data.protein.sites[col].psi
			else
				if modelparams.hidden_conditional_on_aa
...
#=
function getphipsitrans(node::TreeNode, modelparams::ModelParams, phiobserved::Bool=false, psiobserved::Bool=false)
	#obskappa = 0.001
	#diffusionrate = 1.0
	obsinvkappa = modelparams.hiddennodes[1].obsinvkappa
	diffusionrate = modelparams.hiddennodes[1].diffusionrate

	phikappa = min(2000.0, 1.0/(diffusionrate*node.branchlength))
	if phiobserved
		phikappa = min(2000.0, 1.0/(obsinvkappa + (diffusionrate*node.branchlength)))
	end
	phidist = VonMises(0.0, phikappa)

	psikappa = min(2000.0, 1.0/(diffusionrate*node.branchlength))
	if psiobserved
		psikappa = min(2000.0, 1.0/(obsinvkappa + (diffusionrate*node.branchlength)))
	end
	psidist = VonMises(0.0, psikappa)
	return phidist, psidist
end


function branchdiffusionloglikelihood(col::Int, parentnode::TreeNode, node::TreeNode, modelparams::ModelParams, temppath::Array{Array{Int,1},1}=Array{Int,1}[], tempaapath::Array{Array{Int,1},1}=Array{Int,1}[])
	loglikelihood = 0.0
	path = node.data.branchpath.paths[col]
	if length(temppath) > 0
		path = temppath[node.nodeindex]
	end
	aapath = node.data.aabranchpath.paths[col]
	if length(tempaapath) > 0
		aapath = tempaapath[node.nodeindex]
	end
	if length(path) == 1
		parentphi = parentnode.data.protein.sites[col].phi
		parentpsi = parentnode.data.protein.sites[col].psi
		phi = node.data.protein.sites[col].phi
		psi = node.data.protein.sites[col].psi

		phidist, psidist = getphipsitrans(node, modelparams, node.data.protein.sites[col].phiobserved, node.data.protein.sites[col].psiobserved)
		loglikelihood += logpdf(phidist, parentphi-phi)
		loglikelihood += logpdf(psidist, parentpsi-psi)
	end
	loglikelihood += siteloglikelihood(node.data.protein.sites[col], path[end], aapath[end], modelparams, length(path) > 1 || isroot(node), true)
	return loglikelihood
end


function diffusionloglikelihood2(col::Int, nodelist::Array{TreeNode,1}, modelparams::ModelParams, temppath::Array{Array{Int,1},1}=Array{Int,1}[], tempaapath::Array{Array{Int,1},1}=Array{Int,1}[])
	loglikelihood = 0.0	
	for node in nodelist
		if !isroot(node)
			parentnode = get(node.parent)
			if length(temppath) == 0
				loglikelihood += branchdiffusionloglikelihood(col, parentnode, node, modelparams)
			else
				loglikelihood += branchdiffusionloglikelihood(col, parentnode, node, modelparams, temppath, tempaapath)
			end
		end
	end
	return loglikelihood
...
function diffusionloglikelihood(col::Int, nodelist::Array{TreeNode,1}, modelparams::ModelParams, temppath::Array{Array{Int,1},1}=Array{Int,1}[], tempaapath::Array{Array{Int,1},1}=Array{Int,1}[])
	...
	for node in nodelist
		...
		if length(path) == 1 && !isroot(node)
			...
			wnphi = modelparams.hiddennodes[path[end]].phi_diffusion_node
			set_branchlength(wnphi, node.branchlength, site.phiobserved)
			wnpsi = modelparams.hiddennodes[path[end]].psi_diffusion_node
			set_branchlength(wnpsi, node.branchlength, site.psiobserved)
			llphi = logtpd(wnphi, parentphi, phi)
			if llphi == -Inf && node.branchlength == 0.0
				llphi = 0 
			end
			llpsi = logtpd(wnpsi, parentpsi, psi)
			if llpsi == -Inf && node.branchlength == 0.0
				llpsi = 0 
			end
			loglikelihood += llphi + llpsi
		end
		loglikelihood += siteloglikelihood(node.data.protein.sites[col], path[end], aapath[end], modelparams, length(path) > 1 || isroot(node); includesampled=true)
...
function observationloglikelihood(proteins::Array{Protein,1}, nodelist::Array{TreeNode,1}, modelparams::ModelParams)
	...
	numcols = length(nodelist[1].data.protein)
	ll = sum(nodelist[1].data.angleloglikelihood)
	#=
	for col=1:numcols
		#ll += diffusionloglikelihood(col, nodelist, modelparams)
		ll += 
		...
		philikelihoods, psilikelihoods, phill, psill = felsenstein_angles(col, nodelist, modelparams, cache, temppath, tempaapath, getangles(col, nodelist))
		ll += phill+psill		
	end=#
	...
	ll = 0.0
	for node in nodelist
		for col=1:length(node.data.protein)
			if node.data.protein.sites[col].phiobserved && node.data.protein.sites[col].psiobserved
				h = node.data.branchpath.paths[col][end]				 
				ll += siteloglikelihood(proteins[node.seqindex].sites[col], h, node.data.protein.sites[col].aa, modelparams)
			end
		end
	end
	return ll
...
function getangles(col::Int, nodelist::Array{TreeNode,1})
	angles = Tuple{Float64,Float64}[]
	for node in nodelist
		push!(angles, (node.data.protein.sites[col].phi, node.data.protein.sites[col].psi))
	end
	return angles
end


function setangles(col::Int, nodelist::Array{TreeNode,1}, angles::Array{Tuple{Float64,Float64},1})
	for (node,(phi,psi)) in zip(nodelist, angles)
		node.data.protein.sites[col].phi = phi
		node.data.protein.sites[col].psi = psi
	end
end

#=
function calculatetransliks!(likelihoods::Array{Float64,2}, nodeindex::Int, x::Int, leftlikelihoods::Array{Float64,1}, rightlikelihoods::Array{Float64,1}, lefttransdist::VonMises, righttransdist::VonMises, leftpath::Array{Int,1}, rightpath::Array{Int,1})
	numcats = length(leftlikelihoods)
	leftloglik = -Inf
	rightloglik = -Inf
	for y=1:numcats
		deltaangle = indextoangle(x, numcats) - indextoangle(y, numcats)

		transloglik = 0.0
		if length(leftpath) == 1
			transloglik += logpdf(lefttransdist, deltaangle)
			transloglik += log(leftlikelihoods[y])
		end
		leftloglik = logsumexp(leftloglik, transloglik)

		transloglik = 0.0
		if length(rightpath) == 1
			transloglik += logpdf(righttransdist, deltaangle)
			transloglik += log(rightlikelihoods[y])
		end
...
function backwardsampling_angles(rng::AbstractRNG, philikelihoods::Array{Float64,2}, psilikelihoods::Array{Float64,2}, col::Int, nodelist::Array{TreeNode,1}, modelparams::ModelParams, cache::OUDiffusionCache, path::Array{Array{Int,1},1}=Array{Int,1}[], aapath::Array{Array{Int,1},1}=Array{Int,1}[])
	...
	phistates[1] = CommonUtils.sample(rng, philikelihoods[1,:])
	psistates[1] = CommonUtils.sample(rng, psilikelihoods[1,:])
	for node in nodelist
		phistate = phistates[node.nodeindex]
		psistate = psistates[node.nodeindex]
		for child in node
			site =  nodelist[child.nodeindex].data.protein.sites[col]
			
			phisamplevec = philikelihoods[child.nodeindex,:]
			psisamplevec = psilikelihoods[child.nodeindex,:]
			if length(path[child.nodeindex]) == 1
				h = path[child.nodeindex][end]
				phitrans, phiconsts = getphitransitionmatrix(modelparams, cache, h, child, site.phiobserved)
				psitrans, psiconsts = getpsitransitionmatrix(modelparams, cache, h, child, site.psiobserved)				
				phisamplevec = phitrans[phistate,:].*philikelihoods[child.nodeindex,:]
				psisamplevec = psitrans[psistate,:].*psilikelihoods[child.nodeindex,:]
			end
			phistates[child.nodeindex] = CommonUtils.sample(rng, phisamplevec)
			psistates[child.nodeindex] = CommonUtils.sample(rng, psisamplevec)
		end
	end

	phipsiangles = Tuple{Float64,Float64}[]
	delta = 2.0*pi/numcats
	deltadiv2 = delta/2.0
	#println("---------------")
	for (nodeindex, (phiindex,psiindex)) in enumerate(zip(phistates, psistates))
		site =  nodelist[nodeindex].data.protein.sites[col]
		phi = site.phi
		psi = site.psi
		if !site.phiobserved || site.phi < -100.0
			phi = pimod(indextoangle(phiindex, numcats) - deltadiv2 + rand(rng)*delta)
		end
		if !site.psiobserved || site.psi < -100.0
			psi = pimod(indextoangle(psiindex, numcats) - deltadiv2 + rand(rng)*delta)
		end
		push!(phipsiangles, (phi,psi))
		#println((phi,psi),"\t",site.phiobserved,"\t",site.psiobserved)
	end
	#println("*************")
	return phipsiangles
end
...
function felsenstein_angles(col::Int, nodelist::Array{TreeNode,1}, modelparams::ModelParams, cache::OUDiffusionCache, path::Array{Array{Int,1},1}=Array{Int,1}[], aapath::Array{Array{Int,1},1}=Array{Int,1}[], angles::Array{Tuple{Float64,Float64},1}=Tuple{Float64,Float64}[])
	...
	numcats = modelparams.numcats
	philogm = zeros(Float64, length(nodelist))
	psilogm = zeros(Float64, length(nodelist))	
	philikelihoods = ones(Float64, length(nodelist), numcats)*-Inf
	psilikelihoods = ones(Float64, length(nodelist), numcats)*-Inf
	stack = Int[1]
	while length(stack) > 0
		nodeindex = stack[end]
		node = nodelist[nodeindex]
		...
		elseif isleafnode(node)
			if site.phiobserved && site.phi > -100.0
				...
				philikelihoods[nodeindex, angletoindex(site.phi, numcats)] = 1.0
			else
				for z=1:numcats
					philikelihoods[nodeindex, z] = 1.0
				end
			...
			if site.psiobserved && site.psi > -100.0
				...
				psilikelihoods[nodeindex, angletoindex(site.psi, numcats)] = 1.0
			else
				for z=1:numcats
					psilikelihoods[nodeindex, z] = 1.0
				end
			...
			rightchildindex = node.children[2].nodeindex
			cont = true
			if philikelihoods[leftchildindex,1] == -Inf
				push!(stack, leftchildindex)
				cont = false
			end
			if philikelihoods[rightchildindex,1] == -Inf
				push!(stack, rightchildindex)
				cont = false
			end
			...
			if cont
				...
				if calccondloglik			
					...
					if length(path[leftchildindex]) == 1
						v1 = dot(leftphitrans, philikelihoods[leftchildindex,:])
					end
					if length(path[rightchildindex]) == 1	
						v2 = dot(rightphitrans, philikelihoods[rightchildindex,:])
					end
					philikelihoods[nodeindex, x] = log(v1*v2)

					v1 = sum(psilikelihoods[leftchildindex,:])
					v2 = sum(psilikelihoods[rightchildindex,:])
					x = angletoindex(angles[nodeindex][2], numcats)
					leftpsitrans, leftpsiconsts = gettransitionmatrixrow(modelparams.hiddennodes[h].psi_diffusion_node, modelparams.numcats, nodelist[leftchildindex].branchlength, leftsite.psiobserved, x)
					rightpsitrans, rightpsiconsts = gettransitionmatrixrow(modelparams.hiddennodes[h].psi_diffusion_node, modelparams.numcats, nodelist[rightchildindex].branchlength, rightsite.psiobserved, x)
				
					if length(path[leftchildindex]) == 1
						v1 = dot(leftpsitrans, psilikelihoods[leftchildindex,:])
					end
					if length(path[rightchildindex]) == 1	
						v2 = dot(rightpsitrans, psilikelihoods[rightchildindex,:])
					end
					psilikelihoods[nodeindex, x] = log(v1*v2)
				else
					leftphitrans, leftphiconsts = getphitransitionmatrix(modelparams, cache, h, nodelist[leftchildindex], leftsite.phiobserved)
					leftpsitrans, leftpsiconsts = getpsitransitionmatrix(modelparams, cache, h, nodelist[leftchildindex], leftsite.psiobserved)
					...
					if length(path[leftchildindex]) == 1
						v1 = leftphitrans*philikelihoods[leftchildindex,:]
					end
					if length(path[rightchildindex]) == 1	
						v2 = rightphitrans*philikelihoods[rightchildindex,:]
					end
					philikelihoods[nodeindex, :] = log.(v1.*v2)

					v1 = ones(Float64,numcats)*sum(psilikelihoods[leftchildindex,:])
					v2 = ones(Float64,numcats)*sum(psilikelihoods[rightchildindex,:])
					if length(path[leftchildindex]) == 1
						v1 = leftpsitrans*psilikelihoods[leftchildindex,:]
					end
					if length(path[rightchildindex]) == 1	
						v2 = rightpsitrans*psilikelihoods[rightchildindex,:]
					end
					psilikelihoods[nodeindex, :] = log.(v1.*v2)
				end
				...
				if length(path[rightchildindex]) == 1
					philogm[nodeindex] += rightphiconsts
					psilogm[nodeindex] += rightpsiconsts
				end
				...
				philikelihoods[nodeindex,:] = philikelihoods[nodeindex,:] .- m
				philikelihoods[nodeindex,:] = exp.(philikelihoods[nodeindex,:])
				philogm[nodeindex] += m + philogm[leftchildindex] + philogm[rightchildindex]
    ...
    phill = log(sum(philikelihoods)) + philogm[1]
    psill = log(sum(psilikelihoods)) + psilogm[1]
   # println("phi $(col) ", philikelihoods[1,:], "\t", phill)
    #println("psi $(col) ", psilikelihoods[1,:], "\t", psill)
   	...
   	if length(angles) > 0
	    ...
	    for i=1:length(nodelist)
	    	println("phi $(col) ", philikelihoods[i,:], "\t", nodelist[i].data.protein.sites[col].phi, "\t", angles[i][1], "\t", isleafnode(nodelist[i]))
	    end
	    for i=1:length(nodelist)
	    	println("psi $(col) ", psilikelihoods[i,:], "\t", nodelist[i].data.protein.sites[col].psi, "\t", angles[i][2], "\t", isleafnode(nodelist[i]))
	    end
	end=#
    
    return philikelihoods, psilikelihoods, phill, psill
end
...
function calculateloglikelihood(modelparams::ModelParams, trainingexamples::Array{Tuple,1})
	...
	observationll = 0.0
	augmentedll = 0.0
	augmentedll = @distributed (+) for (proteins,nodelist) in trainingexamples
		numcols = length(proteins[1])
		augmentedloglikelihood(nodelist, Int[col for col=1:numcols], modelparams)
	end
	observationll =  @distributed (+) for (proteins,nodelist) in trainingexamples
		numcols = length(proteins[1])
		observationloglikelihood(proteins, nodelist, modelparams)
	end
	=#
...
function correctbounds(lower::Array{Float64,1}, upper::Array{Float64,1}, x::Array{Float64,1})
	ret = zeros(Float64, length(x))
	for (index, (l,u,v)) in enumerate(zip(lower,upper,x))
		ret[index] = max(l, min(u, v))
	end
	return ret
end
...
function diffusionloglikelihood(data::Array{Tuple{Float64,Float64,Float64,Bool,Bool},1}, params::Array{Float64,1})
	...
	for (branchlength, parenttheta, theta, observed, usetpd) in data
		ll = 0.0
		if usetpd
			set_branchlength(dist, branchlength, observed)
			ll = logtpd(dist, parenttheta, theta)
		else
			ll = logstat(dist, theta)
		end
		#=
		if ll == -Inf
			println(branchlength, "\t", parenttheta, "\t", theta, "\t", observed, "\t", usetpd)
		end=#
		if ll == -Inf && branchlength == 0.0
			ll = 0.0
		end
		loglikelihood += ll
	end
...
function optimizediffusion(data::Array{Tuple{Float64,Float64,Float64,Bool,Bool},1}, wn::WrappedUnivariateOUNode; fixevolutionary::Bool=false)
    ...
    inputparams = copy(initparams)
    inputparamsmu = copy(inputparams)
    inputparamsmu[1] = mu
    if diffusionloglikelihood(data,inputparamsmu) > diffusionloglikelihood(data,inputparams)
		initparams = copy(inputparamsmu)
	end
    #println("Initial params: ", initparams, "\t", mu,"\t",std(VonMises(0.0,kappa)),"\t",length(thetas))
    #initparams[1] = mu
    println("initparams: ", initparams)

    localObjectiveFunction = ((params, grad) -> diffusionloglikelihood(data,params))    
	#GN_DIRECT
	#GN_CRS2_LM
	#GN_DIRECT
	opt = Opt(:GN_CRS2_LM, length(lower))   
    lower_bounds!(opt, lower)    
    upper_bounds!(opt, upper)
    xtol_rel!(opt,1e-6)
    maxeval!(opt, 2000)
    max_objective!(opt, localObjectiveFunction)
    (minf,minx,ret) = optimize(opt, correctbounds(lower, upper, initparams))
	set_transformed_parameters(wn,minx)

	if diffusionloglikelihood(data,initparams) > diffusionloglikelihood(data,minx)
		println("Using previous optima")
	else
		initparams = copy(minx)
		println("Using global optima")
	end
    ...
    #println(data[1:100:end])
    println("inputparams:  ",inputparams,"\t",diffusionloglikelihood(data,inputparams))	
	println("initparams:  ",initparams,"\t",diffusionloglikelihood(data,initparams))	
	println("finalparams: ",minx,"\t",diffusionloglikelihood(data,minx))
