module AngleUtils
  ...
  function angular_rmsd(angles1::Array{Tuple{Float64,Float64},1}, angles2::Array{Tuple{Float64,Float64},1})
    ...
    for i=1:length(angles1)
      ...
      psit = angles2[i][2]
      if phi0 > -100.0 && phit > -100.0 && psi0 > -100.0 && psit > -100.0
        dist += sqrt(2.0*(2.0 - cos(phi0-phit) - cos(psi0-psit)))
        len += 1
      end
    end
  ...
  export angular_rmsd_percentile
  function angular_rmsd_percentile(angles1::Array{Tuple{Float64,Float64},1}, angles2::Array{Tuple{Float64,Float64},1}, perc::Float64=90.0)
    dists = Float64[]
    for i=1:length(angles1)
      phi0 = angles1[i][1]
      psi0 = angles1[i][2]
      ...
      psit = angles2[i][2]
      if phi0 > -100.0 && phit > -100.0 && psi0 > -100.0 && psit > -100.0
        push!(dists, sqrt(2.0*(2.0 - cos(phi0-phit) - cos(psi0-psit))))
      end
    end

    if length(dists) > 0
      return StatsBase.percentile(dists, perc)
    else
      return sqrt(8.0)
    end
  end
