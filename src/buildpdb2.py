...
for aa in range(startaa, numaminoacids):
	...
	torsionangle = phi_psi[aa-1][1]+np.pi
	bondangle =  calc_angle(peptidein[aa-1]['CA'].coord, peptidein[aa-1]['C'].coord, peptidein[aa]['N'].coord)+np.pi
	print(R)
	print(bondangle)
	...
	print((bchat,np.cross(n,bchat),n))
	print(M)
	print(D)
	print(np.matmul(M,D))
	print(norm(np.matmul(M,D)))
	#exit()
	N1 = np.matmul(M,D) + C1
	builder.init_atom("N", N1, 1.0, 1.0, " ", " N ")
	...
	bondangle = calc_angle(peptidein[aa-1]['C'].coord, peptidein[aa]['N'].coord, peptidein[aa]['CA'].coord) +np.pi
	...
	torsionangle = phi_psi[aa][0] + np.pi
	...
	bondangle = calc_angle(peptidein[aa]['N'].coord, peptidein[aa]['CA'].coord, peptidein[aa]['C'].coord) + np.pi
	print(R)
	print(bondangle)
	exit()
	D = np.array([R*np.cos(bondangle), R*np.cos(torsionangle)*np.sin(bondangle), R*np.sin(torsionangle)*np.sin(bondangle)])
